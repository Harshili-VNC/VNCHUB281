      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {tab === "overview" && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-3 text-xs">
            ...
            <InfoBox label="Personal Email" value={person.personalEmail ?? "—"} />
            <InfoBox label="Phone Number" value={person.phoneNumber ?? "—"} />
            <InfoBox label="Emergency Contact" value={person.emergencyContactName ? `${person.emergencyContactName} (${person.emergencyContactRelation}) - ${person.emergencyContactPhone}` : "—"} />
            <InfoBox label="Department Function" value={person.departmentFunction} />
            <InfoBox label="Designation Level" value={person.designationLevel ?? "—"} />
            <InfoBox label="Official Location" value={person.officialWorkLocation ?? "India Office"} />
            <InfoBox label="Employee Category" value={person.employeeCategory ?? "Billable"} />
            <InfoBox label="Planning Active" value={person.currentActiveForPlanning ? "Yes" : "No"} />
            <InfoBox label="Role Tags" value={person.roleTags.length ? person.roleTags.join(", ") : "—"} />
          </div>
        )}
        ...
        {tab === "employment" && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-3 text-xs">
            ...
            <InfoBox label="Employment Status" value={person.employmentStatus} />
            <InfoBox label="Annual Salary" value={person.salary ? `₹${person.salary.toLocaleString()}` : "—"} />
            <InfoBox label="Joining CTC" value={person.joiningCtc ? `₹${person.joiningCtc.toLocaleString()}` : "—"} />
            <InfoBox label="Monthly Capacity" value={`${person.standardMonthlyCapacityHours ?? 160} hrs`} />
            <InfoBox label="Project Target Hours" value={`${person.standardProjectHours ?? 120} hrs`} />
          </div>
        )}
        ...
        {tab === "compensation" && (
          <div className="space-y-2 pt-2 text-xs">
            ...
            <div className="p-3 rounded-xl border border-border bg-surface-2 flex items-center justify-between">
              <div>
                <div className="font-semibold text-foreground">Current Effective CTC: ₹{person.salary?.toLocaleString() ?? "9,00,000"}</div>
                <div className="text-muted-foreground">Joining Salary</div>
              </div>
