...
function ClientApprovalsPage() {
  ...
  const allPending = clientsPendingApproval(clients);
  const pending = user && (Boolean(user.isBusinessUnitHead) || canApproveClient(user))
    ? allPending.filter((c) => canApproveClient(user, c))
    : allPending;
