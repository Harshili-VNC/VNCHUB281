...
// Discriminates system-generated Client Master workflow tasks from ordinary
// manager-assigned tasks. Existing rows and the generic addTaskFn are
// unaffected — this defaults to "general" and is purely additive.
...
export const people = pgTable(
  "people",
  {
    ...
    managerId: text("manager_id").references((): AnyPgColumn => people.id),
    hireDate: date("hire_date", { mode: "string" }),
    // System & Technical Columns
    departmentFunction: departmentFunctionEnum("department_function").notNull().default("Operations"),
  ...
  (table) => [
    index("idx_people_manager_id").on(table.managerId),
    index("idx_people_department_id").on(table.departmentId),
    index("idx_people_designation_id").on(table.designationId),
    index("idx_people_primary_bu").on(table.primaryBusinessUnit),
  ],
);
...
export const tasks = pgTable("tasks", {
  ...
  // Client Master workflow linkage (nullable — unused by ordinary tasks).
...
export const clients = pgTable("clients", {
  ...
  clientPhoneCountryCode: text("client_phone_country_code"),
  ...
  clientCountryIso: text("client_country_iso"),
