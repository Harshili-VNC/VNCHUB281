...
import { canOpenClient360 } from "./client-visibility";
import type {
  ClientRecord,
  DocumentRecord,
  ClientChangeRequest,
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const currentUser = (bootstrapQuery.data?.user ?? null) as Person | null;
  ...
  function openClient360(clientOrId: string | ClientRecord) {
    const candidate =
      typeof clientOrId === "string"
        ...
        : clientOrId;
    if (!candidate) return;
    // Defense in depth: the client lists this is called from are already
    // filtered server-side by canViewClient, and getClientDetailsFn now gates
    // its own reads, but the dialog itself previously opened for any record
    // handed to it with no check at all.
    if (!canOpenClient360(currentUser, candidate)) return;
    setSelected360Client(candidate);
  }
  ...
  /**
   * Runs a server mutation and guarantees a resolved Result — never a
   * rejected promise.
   *
   * Every action below used to call its server function bare:
   *   const result = await xFn(...); if (result.ok) await refresh(); return result;
   *
   * If the server function threw (DB error, missing migration, network drop,
   * serialization failure), the promise rejected and every caller's
   * `setBusy(false)` / dialog-close line was skipped — leaving dialogs stuck
   * on "Saving…" indefinitely with no toast. Routing all of them through this
   * wrapper converts any thrown exception into the same
   * `{ ok: false, error }` shape callers already handle, so the UI can always
   * recover and always show a message.
   */
  async function runAction<T extends { ok: boolean }>(
    label: string,
    fn: () => Promise<T>,
    ...
    try {
      ...
      return {
        ok: false as const,
        error: `${label} could not be completed. Please try again or contact support if this persists.${detail}`,
      };
    }
  }

  async function addTask(input: AddTaskInput): Promise<Result> {
    return runAction("Creating the task", () => addTaskFn({ data: input }));
  }

  async function setTaskStatus(id: string, status: TaskStatus): Promise<Result> {
    return runAction("Updating the task", () => setTaskStatusFn({ data: { id, status } }));
  }
  ...
  async function addClient(input: ClientInput): Promise<Result & { id?: string; code?: string }> {
    return runAction("Creating the client", () => addClientFn({ data: input }));
  }
  ...
  async function decideClientApproval(
    ...
    return runAction("Recording the approval decision", () =>
      decideClientApprovalFn({ data: { id, decision, note } }),
    );
  }

  async function addClientChangeRequest(input: ChangeRequestInput): Promise<Result> {
    return runAction("Raising the change request", () =>
      addClientChangeRequestFn({ data: input }),
    );
  }

  async function applyClientChangeRequest(id: string): Promise<Result> {
    return runAction("Applying the change request", () =>
      applyClientChangeRequestFn({ data: { id } }),
    );
  }
  ...
  async function decideClientChangeRequest(
    ...
    return runAction("Recording the change request decision", () =>
      decideClientChangeRequestFn({ data: { id, decision, note } }),
    );
  }

  async function processDueChangeRequests(): Promise<Result & { count?: number }> {
    return runAction("Processing due change requests", () => processDueChangeRequestsFn());
  }

  async function assignClientTeamOwnership(input: TeamOwnershipInput): Promise<Result> {
    return runAction("Saving team ownership", () =>
      assignClientTeamOwnershipFn({ data: input }),
    );
  }

  async function setClientAccounts(clientId: string, accounts: AccountItem[]): Promise<Result> {
    return runAction("Saving client accounts", () =>
      setClientAccountsFn({ data: { clientId, accounts } }),
    );
  }
