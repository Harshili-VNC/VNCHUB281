...
export function isUserInClientBU(user: Person | null, client: ClientRecord | null): boolean {
  ...
  const userPBU = (user.primaryBusinessUnit || "").toLowerCase();
  const userSBU = (user.secondaryBusinessUnit || "").toLowerCase();
  const clientBU = client.businessUnit.toLowerCase();

  const matches = (uBU: string, cBU: string) => {
    if (!uBU || !cBU) return false;
    if (uBU === cBU) return true;
    if (uBU.includes(cBU) || cBU.includes(uBU)) return true;
    const match = uBU.match(/\(([^)]+)\)/);
    if (match && match[1].toLowerCase() === cBU) return true;
    return false;
  };

  return matches(userPBU, clientBU) || matches(userSBU, clientBU);
}
...
export function canReviewClientApproval(user: Person | null, client: ClientRecord | null): boolean {
  ...
  const role = getClientRole(user);
  const isBUHeadFlag = Boolean(user.isBusinessUnitHead);
  if (role !== "Business Unit Head" && !isBUHeadFlag) return false;
