...
/** Standardized UI short form mappings for Client Master table */
export function getShortBUName(bu?: string | null): string {
  if (!bu || !bu.trim()) return "—";
  const b = bu.trim().toLowerCase();
  if (b.includes("enterprise finance") || b.includes("efa")) return "EFA";
  if (b.includes("supply chain") || b.includes("sca")) return "SCA";
  if (b.includes("au & nz") || b.includes("anza")) return "ANZA";
  if (b.includes("managed business") || b.includes("mbs")) return "MBS";
  if (b.includes("corporate") || b.includes("corp")) return "CORP";
  return bu.trim();
}

export function getShortEntityName(entity?: string | null): string {
  if (!entity || !entity.trim()) return "—";
  const e = entity.trim();
  const lower = e.toLowerCase();
  if (lower === "vnc au") return "VNC AU";
  if (lower === "vnc gl" || lower === "vnc global") return "VNC GL";
  if (lower.includes("nth")) return "NTH";
  return e;
}

export function formatShortBUEntity(bu?: string | null, entity?: string | null): string {
  const shortBU = getShortBUName(bu);
  const shortEntity = getShortEntityName(entity);
  return `${shortBU} / ${shortEntity}`;
}
